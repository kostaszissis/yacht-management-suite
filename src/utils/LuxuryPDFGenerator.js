...
export function generateLuxuryPDF(bookingData, mode, additionalData = {}, lang = 'en', options = {}) {
    lang = 'en'; // Force English for all PDFs
  const doc = new jsPDF({
  ...
  if (!options.isPage5 && !options.isPage1 && mode === 'in' && additionalData.equipment && additionalData.equipment.length > 0) {
    ...
    additionalData.equipment.forEach((item) => {
      ...
      doc.setTextColor(...COLORS.black);
      const itemName = item.nameEn || item.name || getItemLabel(item.key, lang) || sanitizeText(item.key) || '';
...
// =================================================================
  ...
  // Page 3 Optional Equipment (in)
  if (!options.isPage5 && options.isPage3 && mode === 'in' && additionalData.optional && additionalData.optional.length > 0) {
    const tableWidth = pageWidth - 2 * margin;
    const col1 = tableWidth * 0.40;
    const col2 = tableWidth * 0.15;
    const col3 = tableWidth * 0.225;
    ...
    doc.setFontSize(14);
    doc.setTextColor(...COLORS.navy);
    doc.text('Optional Equipment', margin, y);
    y += 8;

    doc.setFontSize(11);
    doc.setFont('DejaVuSans', 'normal');

    additionalData.optional.forEach((item) => {
      if (y > pageHeight - 40) {
        doc.addPage();
        y = 20;
      }
      ...
      doc.setTextColor(...COLORS.grey);
      const price = item.price || 0;
      doc.text(`${String.fromCharCode(8364)}${Number(price).toFixed(2)}`, pageWidth - margin - 2, y, { align: 'right' });
  ...
  // Page 3 Cabin Inventory (in)
  if (!options.isPage5 && options.isPage3 && mode === 'in' && additionalData.cabin && additionalData.cabin.length > 0) {
    const tableWidth = pageWidth - 2 * margin;
    const col1 = tableWidth * 0.40;
    const col2 = tableWidth * 0.15;
    const col3 = tableWidth * 0.225;
    ...
    doc.setFontSize(14);
    doc.setTextColor(...COLORS.navy);
    doc.text('Cabin Inventory', margin, y);
    y += 8;

    doc.setFontSize(11);
    doc.setFont('DejaVuSans', 'normal');

    additionalData.cabin.forEach((item) => {
      if (y > pageHeight - 40) {
        doc.addPage();
        y = 20;
      }
  ...
  // Page 3 Optional Equipment (out)
  if (!options.isPage5 && options.isPage3 && mode === 'out' && additionalData.optional && additionalData.optional.length > 0) {
    const tableWidth = pageWidth - 2 * margin;
    const col1 = tableWidth * 0.40;
    const col2 = tableWidth * 0.15;
    const col3 = tableWidth * 0.225;
  ...
  // Page 3 Cabin Inventory (out)
  if (!options.isPage5 && options.isPage3 && mode === 'out' && additionalData.cabin && additionalData.cabin.length > 0) {
    const tableWidth = pageWidth - 2 * margin;
    const col1 = tableWidth * 0.40;
    const col2 = tableWidth * 0.15;
    const col3 = tableWidth * 0.225;
      ...
      if (mode === 'out') {
        ...
        if (damagedItems.length > 0) {
          ...
          damagedItems.forEach((item) => {
            ...
            doc.setFontSize(10);
            doc.setTextColor(...COLORS.black);
            const itemName = sanitizeText(item.nameEn || item.name) || '';
