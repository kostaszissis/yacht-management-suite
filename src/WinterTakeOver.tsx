...
import { getWinterTakeover, saveWinterTakeover } from './services/apiService';
...
// Section definitions for take-over checklist
const SECTIONS = {
  inventory: {
    id: 'inventory',
    icon: '📦',
    titleEl: 'INVENTORY CHECK',
    titleEn: 'INVENTORY CHECK',
    defaultItems: [
      'Κλινοσκεπάσματα',
      'Πετσέτες',
      'Μαγειρικά Σκεύη',
...
const STORAGE_KEY = 'winter_take_over_v3';
const CUSTOM_SECTIONS_KEY = 'winter_take_over_custom_sections';
...
const WinterTakeOver: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const lastVessel = localStorage.getItem('takeover_last_vessel');
    if (lastVessel) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('takeover_last_vessel', String(selectedVessel));
    ...
    const loadData = async () => {
      ...
      try {
        // Try API first
        const apiData = await getWinterTakeover(selectedVessel);

        if (apiData && apiData.sections) {
          console.log('✅ Winter TakeOver loaded from API for vessel', selectedVessel);
  ...
  const totalItems = sectionValues.reduce((acc: number, s: SectionState) => acc + s.items.length, 0);
  const completedItems = sectionValues.reduce((acc: number, s: SectionState) => acc + s.items.filter((i: TaskItem) => i.status === 'OK').length, 0);
  const pendingItems = sectionValues.reduce((acc: number, s: SectionState) => acc + s.items.filter((i: TaskItem) => i.status === '#').length, 0);
  const repairItems = sectionValues.reduce((acc: number, s: SectionState) => acc + s.items.filter((i: TaskItem) => i.status === '?').length, 0);
  ...
  const removeItem = (sectionId: string, itemId: string) => {
    if (!window.confirm(lang === 'el' ? 'Διαγραφή εργασίας;' : 'Delete task?')) return;
    setSections(prev => ({
      ...prev,
      [sectionId]: {
        ...prev[sectionId],
  ...
  const handleSave = async () => {
    ...
    // Try to save to API
    try {
      const result = await saveWinterTakeover(selectedVessel, sections, customSections, generalNotes);
      if (result.synced) {
        console.log('✅ Winter TakeOver saved to API');
      } else {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Take Over');
    XLSX.writeFile(wb, `TakeOver_${vesselName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  // Render section
  const renderSection = (sectionDef: typeof SECTIONS.inventory) => {
    const section = sections[sectionDef.id];
      ...
      <div className="p-4 rounded-b-xl shadow-md" style={{ background: 'linear-gradient(135deg, #90caf9 0%, #64b5f6 100%)' }}>
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-2xl font-bold text-blue-800 text-center drop-shadow-md">
            📋 {lang === 'el' ? 'ΧΕΙΜΕΡΙΝΕΣ ΕΡΓΑΣΙΕΣ - TAKE OVER' : 'WINTER TASKS - TAKE OVER'}
          </h1>
          <p className="text-blue-700 text-center mt-1">
            {lang === 'el' ? 'Παράδοση - Παραλαβή Σκάφους' : 'Vessel Handover'}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-4">
        ...
        <div className="rounded-xl p-3 mb-4 border border-blue-300 shadow-md flex flex-wrap gap-4 justify-center" style={{ background: 'linear-gradient(135deg, #90caf9 0%, #64b5f6 100%)' }}>
          ...
          <span className="flex items-center gap-2">
            ...
            <span className="text-blue-700 font-bold text-sm">{lang === 'el' ? 'Έλεγχος' : 'Check'}</span>
          </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Left Column */}
          <div>
            {renderSection(SECTIONS.inventory)}
            {renderSection(SECTIONS.technical)}
            {renderSection(SECTIONS.cleaning)}
            {renderSection(SECTIONS.documents)}
            {renderSection(SECTIONS.aircon)}
          </div>
          {/* Right Column */}
          <div>
            {renderSection(SECTIONS.safety)}
            {renderSection(SECTIONS.engine)}
            {renderSection(SECTIONS.hull)}
            {renderSection(SECTIONS.electronics)}
          </div>
        </div>
