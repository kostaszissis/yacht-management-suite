...
// =====================================================
import { translateEmailHtml, translateEmailText } from './emailHelpers';
...
export async function sendCheckInEmail(
  ...
  try {
    ...
    // Customer gets separate translated email
    ...
    if (inventoryItems.length > 0) {
      ...
      inventoryItems.forEach((item: any) => {
        const sectionKey = `${item.page || ''} - ${item.sectionEn || item.section || ''}`;
        if (sectionKey !== currentSection && item.page && (item.sectionEn || item.section)) {
          ...
          tableRows += `
            <tr>
              <td colspan="5" style="background-color: ${COLORS.lightGrey}; padding: 6px 8px; font-size: 9px; font-weight: bold; color: ${COLORS.navy};">
                ${item.page} - ${item.sectionEn || item.section}
              </td>
            </tr>`;
        }
        ...
        tableRows += `
          <tr>
            ...
            <td style="padding: 6px 8px; border-bottom: 1px solid ${COLORS.lightGrey}; font-size: 10px; color: ${COLORS.black};">${item.nameEn || item.name || ''}</td>
...
${inventoryItems.map((item: any) => {
  const isOK = item.inOk || item.in === 'ok';
  const status = isOK ? '✓' : '✗';
  return `${status} ${item.nameEn || item.name || ''} (Qty: ${item.qty || 1}) - €${(parseFloat(item.price) || 0).toFixed(2)}`;
}).join('\n')}
` : ''}
...
Document generated on ${new Date(timestamp).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}`;
    ...
    // ====== Send CUSTOMER email (translated to their language) ======
    if (customerEmail) {
      try {
        let customerHtml = translateEmailHtml(htmlContent, lang);
        let customerText = translateEmailText(textContent, lang);
        // Swap English item names back to translated for customer
        const custItems = additionalData?.allItems || [];
        custItems.forEach((itm: any) => {
          if (itm.nameEn && itm.name && itm.nameEn !== itm.name) {
            // customerHtml = customerHtml.split(itm.nameEn).join(itm.name); // Disabled - all English
            // customerText = customerText.split(itm.nameEn).join(itm.name); // Disabled - all English
          }
        });
        console.log('\u{1F4E7} Sending customer check-in email to:', customerEmail, 'lang:', lang);
        const custResponse = await fetch('https://yachtmanagementsuite.com/email/send-email', {
          method: 'POST',
          mode: 'cors',
          ...
          body: JSON.stringify({
            ...
            html: customerHtml,
            text: customerText,
            attachments: [{ filename: 'check-in-report.pdf', content: pdfBase64, contentType: 'application/pdf' }]
          })
        });
        if (custResponse.ok) {
          console.log('\u{1F4E7} \u2705 Customer check-in email sent to:', customerEmail);
        } else {
          console.error('\u{1F4E7} \u274C Customer check-in email failed:', custResponse.status);
        }
      } catch (custError) {
        console.error('\u{1F4E7} \u274C Customer check-in email error:', custError);
      }
    }
...
${damagedItems.map((item: any) => {
  ...
  const unitPrice = parseFloat(item.price) || 0;
  const total = qty * unitPrice;
  return `✘ ${item.nameEn || item.name || ''} (Qty: ${qty}) - €${total.toFixed(2)}`;
}).join('\n')}
...
Document generated on ${new Date(timestamp).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}`;
    ...
    if (customerEmail) {
      try {
        ...
        console.log('\u{1F4E7} Sending customer check-out email to:', customerEmail, 'lang:', lang);
        const custResponse = await fetch('https://yachtmanagementsuite.com/email/send-email', {
          method: 'POST',
          mode: 'cors',
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          body: JSON.stringify({
            ...
            html: customerHtml,
            text: customerText,
            attachments: [{ filename: 'check-out-report.pdf', content: pdfBase64, contentType: 'application/pdf' }]
          })
        });
        if (custResponse.ok) {
          console.log('\u{1F4E7} \u2705 Customer check-out email sent to:', customerEmail);
        } else {
          console.error('\u{1F4E7} \u274C Customer check-out email failed:', custResponse.status);
        }
      } catch (custError) {
        console.error('\u{1F4E7} \u274C Customer check-out email error:', custError);
      }
    }
