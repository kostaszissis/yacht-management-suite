 ...
 */
// CHARTERER_DATA_MARKER
export async function updateCharterCrew(
  bookingNumber: string,
  crewMembersOrPayload: Array<{
    name: string;
    passport: string;
    dateOfBirth?: string;
    nationality?: string;
  }> | { crewMembers?: any[]; chartererData?: any; skipperLicense?: string; extraLicenses?: string[] },
  extraPayload?: { chartererData?: any; skipperLicense?: string; extraLicenses?: string[] }
) {
  // Backward-compatible: accept old signature (array) OR new (object)
  let bodyPayload: any = { bookingNumber };
  if (Array.isArray(crewMembersOrPayload)) {
    bodyPayload.crewMembers = crewMembersOrPayload;
    if (extraPayload) {
      if (extraPayload.chartererData) bodyPayload.chartererData = extraPayload.chartererData;
      if (extraPayload.skipperLicense) bodyPayload.skipperLicense = extraPayload.skipperLicense;
      if (extraPayload.extraLicenses) bodyPayload.extraLicenses = extraPayload.extraLicenses;
    }
  } else {
    if (crewMembersOrPayload.crewMembers) bodyPayload.crewMembers = crewMembersOrPayload.crewMembers;
    if (crewMembersOrPayload.chartererData) bodyPayload.chartererData = crewMembersOrPayload.chartererData;
    if (crewMembersOrPayload.skipperLicense) bodyPayload.skipperLicense = crewMembersOrPayload.skipperLicense;
    if (crewMembersOrPayload.extraLicenses) bodyPayload.extraLicenses = crewMembersOrPayload.extraLicenses;
  }

  console.log('👥 Saving to bookings API...', bodyPayload);
...
/**
 * 📋 Load charterer data from charter-archive.php
 * @param bookingNumber - The booking number/code
 * @returns chartererData object or null if not found
 */
export async function loadChartererData(bookingNumber: string) {
  console.log('📋 Loading charterer data from API...', { bookingNumber });
  try {
    const response = await fetch(`${API_URL}/charter-archive.php?booking_number=${encodeURIComponent(bookingNumber)}`);
    if (!response.ok) {
      console.log('ℹ️ No archive found for booking');
      return null;
    }
    const result = await response.json();
    const archiveData = result.data || {};
    const chartererData = archiveData.chartererData || null;
    if (chartererData) {
      console.log('✅ Charterer data loaded:', chartererData);
    } else {
      console.log('ℹ️ No charterer data in archive yet');
    }
    return chartererData;
  } catch (error) {
    console.error('❌ Error loading charterer data:', error);
    return null;
  }
}

/**
 * 📋 Save charterer data to charter-archive.php
 * Uses JSONB merge — preserves documents, crewMembers, and other keys
 * @param bookingNumber - The booking number/code
 * @param chartererData - Full charterer object with signature, timestamps, etc.
 * @returns Response from API
 */
export async function saveChartererData(bookingNumber: string, chartererData: any) {
  console.log('📋 Saving charterer data to API...', { bookingNumber });
  const response = await fetch(`${API_URL}/charter-archive.php`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  if (!response.ok) {
    throw new Error('Failed to save charterer data');
  }
  console.log('✅ Charterer data saved successfully');
  return response.json();
}
...
// Fuzzy search booking by numeric code (tolerant of CHARTER PARTY / NO / typos)
export async function searchBookingByCode(code: string): Promise<{ booking_number: string; booking_data: any } | null> {
  if (!code) return null;
  try {
    const res = await fetch(`${API_BASE}/bookings.php?action=search_by_code&code=${encodeURIComponent(code)}`);
