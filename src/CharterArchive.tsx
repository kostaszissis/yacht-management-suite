...
const CharterArchive: React.FC<CharterArchiveProps> = ({ onClose }) => {
  ...
  // Download document
  const downloadDocument = (docType: string, index: number) => {
    const docs = (archiveData?.documents as any)?.[docType] || [];
    const doc = docs[index];
    if (doc?.dataUrl) {
  ...
  // Print document
  const printDocument = (docType: string, index: number) => {
    const docs = (archiveData?.documents as any)?.[docType] || [];
    const doc = docs[index];
    if (doc?.dataUrl) {
      ...
      if (printWindow) {
        if (doc.dataUrl.startsWith('data:application/pdf') || doc.name?.endsWith('.pdf')) {
          printWindow.document.write('<iframe src="' + doc.dataUrl + '" width="100%" height="100%" style="border:none;"></iframe>');
        } else {
          printWindow.document.write('<img src="' + doc.dataUrl + '" style="max-width:100%;" onload="window.print();"/>');
        }
  ...
  const deleteDocument = (docType: string, index: number) => {
    if (!window.confirm('Διαγραφή εγγράφου; / Delete document?')) return;
    setArchiveData(prev => {
      const updated = { ...prev };
              ...
              <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-lg border border-[#e5e7eb]">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    <button
                      onClick={() => downloadDocument(docType, idx)}
                      className="bg-[#2563eb] text-white px-2 py-1 rounded text-xs hover:bg-blue-700 transition"
                    >
                      ⬇
                    </button>
                    <button
                      onClick={() => printDocument(docType, idx)}
                      className="bg-[#7c3aed] text-white px-2 py-1 rounded text-xs hover:bg-purple-700 transition"
                    >
                      🖨
                    </button>
                  <button
