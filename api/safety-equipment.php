<?php
/**
 * SAFETY EQUIPMENT API
 * Handles: winter safety equipment data per vessel
 */

require_once __DIR__ . '/db_connect.php';

// Auto-create table if it doesn't exist
function ensureSafetyEquipmentTable($pdo) {
    $pdo->exec("CREATE TABLE IF NOT EXISTS safety_equipment (
    ...
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_safety_equipment_vessel_id ON safety_equipment(vessel_id)");
}

try {
    $pdo = getDbConnection();
    ensureSafetyEquipmentTable($pdo);
    ...
    switch ($method) {
        case 'GET':
            if (!$vesselId) {
                $stmt = $pdo->query("SELECT * FROM safety_equipment ORDER BY vessel_id");
                ...
                $stmt = $pdo->prepare("SELECT * FROM safety_equipment WHERE vessel_id = :vessel_id");
        ...
        case 'PUT':
            ...
            $checkStmt = $pdo->prepare("SELECT id FROM safety_equipment WHERE vessel_id = :vessel_id");
            $checkStmt->execute(['vessel_id' => $vesselId]);

            if ($checkStmt->fetch()) {
                $stmt = $pdo->prepare("
                    UPDATE safety_equipment SET
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO safety_equipment (vessel_id, vessel_name, sections, custom_sections, general_notes, last_saved)
        ...
        case 'DELETE':
            ...
            $stmt = $pdo->prepare("DELETE FROM safety_equipment WHERE vessel_id = :vessel_id");
            $stmt->execute(['vessel_id' => $vesselId]);
