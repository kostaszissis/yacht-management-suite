<?php
/**
 * WINTER INVENTORY API
 * Handles: winter maintenance inventory data per vessel
 */
...
try {
    $pdo = getDbConnection();
    ensureWinterInventoryTable($pdo);
    ...
    switch ($method) {
        case 'GET':
            if (!$vesselId) {
                $stmt = $pdo->query("SELECT * FROM winter_inventory ORDER BY vessel_id");
                ...
                $stmt = $pdo->prepare("SELECT * FROM winter_inventory WHERE vessel_id = :vessel_id");
        ...
        case 'PUT':
            ...
            $checkStmt = $pdo->prepare("SELECT id FROM winter_inventory WHERE vessel_id = :vessel_id");
            $checkStmt->execute(['vessel_id' => $vesselId]);

            if ($checkStmt->fetch()) {
                $stmt = $pdo->prepare("
                    UPDATE winter_inventory SET
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO winter_inventory (vessel_id, vessel_name, sections, custom_sections, general_notes, last_saved)
        ...
        case 'DELETE':
            ...
            $stmt = $pdo->prepare("DELETE FROM winter_inventory WHERE vessel_id = :vessel_id");
            $stmt->execute(['vessel_id' => $vesselId]);
