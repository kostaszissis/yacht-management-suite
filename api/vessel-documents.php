...
switch ($method) {
    case 'GET':
        if (isset($_GET['vessel_name'])) {
            ...
            $stmt = $pdo->prepare("SELECT id, vessel_name, document_type, document_name, expiry_date, file_name, file_data, notes FROM vessel_documents WHERE vessel_name = ? ORDER BY document_type");
            $stmt->execute([$vessel_name]);
            $documents = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode(['success' => true, 'data' => $documents], JSON_INVALID_UTF8_SUBSTITUTE);
        } else {
            $stmt = $pdo->query("SELECT id, vessel_name, document_type, document_name, expiry_date, file_name, file_data, notes FROM vessel_documents ORDER BY vessel_name, document_type");
            $documents = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode(['success' => true, 'data' => $documents], JSON_INVALID_UTF8_SUBSTITUTE);
        }
        break;
